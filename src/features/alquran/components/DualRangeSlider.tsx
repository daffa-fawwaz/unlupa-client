...
export const DualRangeSlider = ({
  ...
  return (
    <div className="mb-6">
      <div className="flex justify-between font-mono text-sm text-muted-foreground mb-2">
        <span>{label}</span>
        <span>
          <span className="text-warning font-bold text-lg">{lower}</span>
          {unit} s.d{" "}
          <span className="text-warning font-bold text-lg">{upper}</span>
          {unit}
        </span>
      </div>
      ...
      <div className="relative w-full h-12 px-2.5">
        {/* Track */}
        <div className="absolute top-1/2 left-2.5 right-2.5 h-1 bg-surface-1 rounded-full -translate-y-1/2" />

        {/* Range highlight */}
        <div
          className="absolute top-1/2 h-1 bg-warning rounded-full -translate-y-1/2 pointer-events-none z-10"
          style={{
        ...
        <input
          ...
          className="absolute top-1/2 left-2.5 w-[calc(100%-20px)] -translate-y-1/2 z-20 appearance-none bg-transparent pointer-events-auto cursor-pointer
            [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:bg-surface-1 [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-warning [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:cursor-pointer
            [&::-moz-range-thumb]:w-5 [&::-moz-range-thumb]:h-5 [&::-moz-range-thumb]:bg-surface-1 [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-warning [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:cursor-pointer"
        />
