...
export const updateItemReviewCountInCache = (
  bookId: string,
  itemId: string,
  reviewCount: number,
) => {
  ...
  const updateItems = (items: BookItem[]) =>
    items.map((i) =>
      i.id === itemId ? { ...i, review_count: reviewCount } : i,
    );
...
export const updateItemStatusInCache = (
  bookId: string,
  itemId: string,
...
export const useBookTree = () => {
  ...
  const fetchBookTree = useCallback(
    async (bookId: string, forceRefresh = false): Promise<BookTree | null> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to fetch book tree";
        setError(errorMessage);
        console.error("Gagal mengambil struktur kitab");
        return null;
      } finally {
        setLoading(false);
      }
  ...
  const getBookTree = useCallback(
    async (bookId: string): Promise<BookTree | null> => {
  ...
  const addItemToTree = useCallback(
    (bookId: string, newItem: BookTree["items"][0]) => {
      const cached = bookTreeCache.get(bookId);
      if (cached) {
  ...
  const addItemToModule = useCallback(
    (bookId: string, moduleId: string, newItem: BookItem) => {
      const cached = bookTreeCache.get(bookId);
      if (cached) {
        const updateModules = (modules: Module[]): Module[] =>
          modules.map((m) => {
  ...
  const addChildModuleToTree = useCallback(
    (bookId: string, parentId: string, newModule: Module) => {
      const cached = bookTreeCache.get(bookId);
      if (cached) {
        const updateModules = (modules: Module[]): Module[] =>
          modules.map((m) => {
            if (m.id === parentId) {
              return {
                ...m,
                children: appendUniqueById(m.children, newModule),
              };
            }
            if (m.children?.length) {
              return { ...m, children: updateModules(m.children) };
