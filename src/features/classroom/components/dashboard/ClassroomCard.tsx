...
export const ClassroomCard = ({
  ...
  return (
    <article
      onClick={onClick}
      className={`group relative flex w-full max-w-full min-h-105 flex-col overflow-hidden rounded-xl border border-border bg-card transition-colors duration-300 hover:-translate-y-1 hover:border-primary/40 ${onClick ? "cursor-pointer" : ""}`}
    >
      <div className={`absolute inset-x-0 top-0 z-20 h-1 ${theme.accent}`} />

      <div className="relative h-44 shrink-0 overflow-hidden border-b border-border bg-surface-1 sm:h-48">
        {coverImage ? (
          <img
          ...
          <div className={`flex h-full w-full items-center justify-center ${theme.softBg}`}>
            <div
              className={`relative flex h-24 w-24 items-center justify-center rounded-4xl border ${theme.border} ${theme.iconBg} transition-transform duration-700 group-hover:scale-105 group-hover:-rotate-2 sm:h-28 sm:w-28`}
            >
              <GraduationCap
        ...
        <div className="absolute inset-x-0 bottom-0 h-24 bg-linear-to-t from-card via-card/45 to-transparent pointer-events-none" />

        <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-widest ${theme.border} ${theme.softBg} ${theme.text}`}
          >
      ...
      <div className="relative z-10 flex flex-1 flex-col p-5">
        ...
        <div className="mb-2">
          <h3 className="mb-2 line-clamp-2 text-xl font-black leading-tight text-foreground transition-colors group-hover:text-primary">
            {title}
          </h3>
        ...
        {isTeacher ? (
          ...
          <div className="mb-2 grid grid-cols-1 gap-2">
            <div className="rounded-2xl px-6 border flex justify-between items-center border-border bg-surface-1 p-3">
              <div>
                <p className="text-lg font-black leading-none text-foreground">
                  {bookCount}
                </p>
